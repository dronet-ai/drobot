...
threshold=0.5 # (drone - waypoint) < threshold ==> switch to next waypoint
...
class PositionController:
    ...
    def compute_thrust_VS(self,vertical_error,delta_t):
        ...
        self.error_vertical_integral+=error_vetical*delta_t
    ...
    def compute_roll_VS(self,horizontal_error,delta_t):
        ...
        roll= kp_roll*error_horizontal + ki_roll*self.error_horizontal_integral         + kd_roll*error_horizontal_derivative
...
if __name__ == '__main__':
    ...
    gate_waypoints[1] = gate_waypoints[1][0],gate_waypoints[1][1],gate_waypoints[1][2] + 2
    #gate_waypoints = [[18.006790182275775, -10, 7.306497500000001]]
    ...
    csvSlam= open("Slam.csv","a+")
    csvImu=open("imu.csv","a+")
    csvGt = open("gt.csv","a+")
    csvGtA= open("gt_angular.csv","a+")


    writer_slam= csv.writer(csvSlam)
    writer_imu=csv.writer(csvImu)
    writer_gt=csv.writer(csvGt)
    writer_angular=csv.writer(csvGtA)
    new=False
    first=True
    #gate_waypoints=[[18.0, -23.0,6.3 ], (17.251109999999997, 3.6314985, 6.306497500000001), (16.791805, 39.442245, 6.7624995), (16.791805-7, 39.442245, 6.7624995), (2.0891957999999997, 27.86797, 2.54649985), (1.0998320000000002, 11.801728, 2.50375), (-7.3086705, -12.13678, 3.229941), (-8.498042, -29.359895, 2.6425), (-0.00900054, -34.513, 2.1031115000000002),  (7.7508205, -29.32303, 2.56749995), (7.249821000000001, -11.79203, 2.54649985), (-10.598669499999999, 6.203225500000001, 2.0199409999999998),  (-10.048669, 30.623224999999998, 2.897941)]
    gate_waypoints=[[18.0, -23.0,6.3 ], (17.251109999999997, 3.6314985, 6.306497500000001), (16.791805, 37.442245, 6.7624995), (16.791805-7, 39.442245, 6.7624995), (2.0891957999999997, 27.86797, 2.54649985), (1.0998320000000002, 11.801728, 2.50375), (-7.3086705, -12.13678, 3.229941), (-8.498042, -29.359895, 2.6425), (-0.00900054, -34.513, 2.1031115000000002),  (7.7508205, -29.32303, 2.56749995), (7.249821000000001, -11.79203, 2.54649985), (-10.598669499999999, 6.203225500000001, 2.0199409999999998),  (-10.048669, 30.623224999999998, 2.897941)] 
    while not rospy.is_shutdown():
        ...
        try:
            ...
            slam= tfBuffer.lookup_transform("map", 'camera_link', rospy.Time())
        ...
        slam_data=slam_pose(slam)
        imu_data=imu_pose(fused_transform)
        
        ########### get the tf data
        x,y,z,q1,q2,q3,q4=uav_groundtruth_pose(trans)
	#yaw=toEulerAngle(q1,q2,q3,q4)[2]
	x1 = slam_data[0]*cos(Y_i) + slam_data[1]*sin(Y_i) + initial_pose[0]
	y1 = -slam_data[0]*sin(Y_i) + slam_data[1]*cos(Y_i) + initial_pose[1]
	z1 = initial_pose[2] + slam_data[2] -1
	#[q1,q2,q3,q4] = imu_data   
        ...
        slam_transformed=[]
        slam_transformed.append(slam_data[0]*cos(Y_i) + slam_data[1]*sin(Y_i) + initial_pose[0])  #18-y_s
        slam_transformed.append(-slam_data[0]*sin(Y_i) + slam_data[1]*cos(Y_i) + initial_pose[1])   #-23 +x_s
        slam_transformed.append(initial_pose[2] + slam_data[2] -1)    #5.3 + z_s
	slam_transformed = [x1,y1,z1]
	#print("Slam : ",slam_transformed)

        #print(trans.header.stamp)
        gt_data=[x,y,z]
        gt_angular=[q1,q2,q3,q4]
	...
	if new :
	    if first :
		first_stamp=trans.header.stamp
		first=False
	    gt_data=[x,y,z,trans.header.stamp-first_stamp]
            writer_slam.writerow(slam_transformed)
            writer_imu.writerow(imu_data)
            writer_gt.writerow(gt_data)
            writer_angular.writerow(gt_angular) 
